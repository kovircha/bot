...
class GameMiddleware(BaseMiddleware):
    async def __call__(self, handler, event: Message, data: dict):
        ...
        if CONSOLE_LOGS:
            ...
            print(
                f"{Style.DIM}[{t}]{Style.RESET_ALL} {Fore.MAGENTA}{user.id}{Style.RESET_ALL} | {Fore.WHITE}{user.full_name}{Style.RESET_ALL} -> {color}{content}{Style.RESET_ALL}"
            )
        ...
        try:
            ...
            if chat_member.status in ["left", "kicked"]:
                ...
                if (
                    ...
                    await event.answer(
                        "❌ Вы еще не подписались! Проверьте подписку.", show_alert=True
                    )
                    return
                ...
                if isinstance(event, Message):
                    ...
                    await event.message.answer(
                        "🔒 Для продолжения подпишись на канал!", reply_markup=kb
                    )
                    await event.answer()
        ...
        if user.id in muted_users:
            if current_time < muted_users[user.id]:
                return
            else:
                del muted_users[user.id]

        if user.id not in user_timestamps:
            user_timestamps[user.id] = []
        user_timestamps[user.id] = [
            t for t in user_timestamps[user.id] if current_time - t < 1.0
        ]
        ...
        if len(user_timestamps[user.id]) > SPAM_LIMIT:
            ...
            if isinstance(event, Message):
                await event.answer(
                    f"⛔️ <b>Остынь!</b> Мут на {MUTE_TIME} сек.", parse_mode="HTML"
                )
            elif isinstance(event, CallbackQuery):
                await event.answer(
                    f"⛔️ Остынь! Мут на {MUTE_TIME} сек.", show_alert=True
                )
            return
        ...
        valid_buttons = [
            ...
            "🧬 Лаборатория",
            "💲 Торговец",
            "⚖️ Биржа Игроков🔄 Обновить данные",
        ]

        if (
            isinstance(event, Message)
            and not event.text.startswith("/")
            and event.text not in valid_buttons
        ):
...
URL_SCARECROW_GOOD = (
    "https://i.ibb.co/9V40K5z/scarecrow-good.jpg"  # Пример URL-заглушки
)
...
async def sell_init(cb: CallbackQuery, state: FSMContext):
    ...
    await cb.message.answer(
        f"💰 За сколько помидоров ты хочешь продать <b>{card_name}</b>?\n\n✍️ <i>Напиши цену числом (например: 1000)</i>",
        parse_mode="HTML",
    )
    await cb.answer()
...
async def sell_confirm(message: types.Message, state: FSMContext):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        await db.execute(
            "INSERT INTO market (seller_id, seller_name, card_id, price) VALUES (?, ?, ?, ?)",
            (user_id, username, card_id, price),
        )
        await db.commit()

    await message.answer(
        f"✅ Лот создан! <b>{CARDS[card_id]['name']}</b> выставлен за {price} 🍅.",
        parse_mode="HTML",
    )
    await state.clear()
...
async def send_card_info(message: types.Message, card_id: str, count: int = 1):
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="💰 Продать карту", callback_data=f"sell_init_{card_id}"
                )
            ]
        ]
    )
...
async def get_market_page(page: int = 0):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        async with db.execute(
            "SELECT lot_id, seller_name, card_id, price, seller_id FROM market ORDER BY lot_id DESC LIMIT ? OFFSET ?",
            (LIMIT, offset),
        ) as c:
...
async def show_market_page(message_or_call, page=0):
    ...
    rarity_data = RARITY_INFO.get(
        card_info.get("rarity", "common"), RARITY_INFO["common"]
    )
    ...
    if user_id == seller_id:
        buy_btn = InlineKeyboardButton(
            text="🗑 Удалить лот", callback_data=f"market_delete_{lot_id}"
        )
    else:
        buy_btn = InlineKeyboardButton(
            text=f"💳 Купить ({format_num(price)})", callback_data=f"buy_lot_{lot_id}"
        )
    ...
    buttons.append(
        [
            InlineKeyboardButton(
                text="🔄 Обновить список", callback_data=f"market_page_{page}"
            )
        ]
    )
...
async def market_delete_own(cb: CallbackQuery):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        async with db.execute(
            "SELECT card_id FROM market WHERE lot_id = ? AND seller_id = ?",
            (lot_id, user_id),
        ) as c:
            row = await c.fetchone()
        ...
        if exists:
            await db.execute(
                "UPDATE user_cards SET count = count + 1 WHERE user_id = ? AND card_id = ?",
                (user_id, card_id),
            )
        else:
            await db.execute(
...
async def buy_lot(cb: CallbackQuery):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        async with db.execute(
            "SELECT seller_id, card_id, price FROM market WHERE lot_id = ?", (lot_id,)
        ) as c:
            lot = await c.fetchone()
        ...
        if not lot:
            ...
            try:
                await cb.message.delete()  # Удаляем устаревшее сообщение
            except:
                pass
            return
        ...
        async with db.execute(
            "SELECT tomatoes FROM users WHERE user_id = ?", (buyer_id,)
        ) as c:
        ...
        await db.execute(
            "UPDATE users SET tomatoes = tomatoes - ? WHERE user_id = ?",
            (price, buyer_id),
        )
    ...
    await cb.message.answer(
        f"🎉 Ты купил <b>{CARDS[card_id]['name']}</b> за {price} 🍅!", parse_mode="HTML"
    )
    ...
    try:
        await bot.send_message(
            seller_id,
            f"🤑 Твой лот <b>{CARDS[card_id]['name']}</b> купили за {price} 🍅!",
        )
    except:
        pass
...
async def easter_egg_linux(message: types.Message):
    await message.answer(
        "🤖 <i>Kernel panic... System failure...</i>\n\nШучу. Не ломай меня.",
        parse_mode="HTML",
    )
...
async def init_db():
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        await db.execute(
            "CREATE TABLE IF NOT EXISTS user_cards (user_id INTEGER, card_id TEXT, count INTEGER DEFAULT 0, PRIMARY KEY (user_id, card_id))"
        )
        await db.execute(
            "CREATE TABLE IF NOT EXISTS promo_codes (code TEXT PRIMARY KEY, uses_left INTEGER, reward_type TEXT, reward_amount INTEGER)"
        )
        await db.execute(
            "CREATE TABLE IF NOT EXISTS used_codes (user_id INTEGER, code TEXT, PRIMARY KEY (user_id, code))"
        )
        await db.execute(
            "CREATE TABLE IF NOT EXISTS market (lot_id INTEGER PRIMARY KEY AUTOINCREMENT, seller_id INTEGER, seller_name TEXT, card_id TEXT, price INTEGER)"
        )
        await db.commit()
        ...
        new_columns = [
            ...
            (
                "active_boost",
                "TEXT DEFAULT ''",
            ),  # Тип активного буста (milk_x2, water_free и т.д.)
            ...
            (
                "last_tractor_collect",
                "REAL DEFAULT 0",
            ),  # <-- НОВОЕ: Время последнего сбора
        ]
        ...
        current_time = time.time()
        await db.execute(
            "UPDATE users SET reg_date = ? WHERE reg_date = 0", (current_time,)
        )
        await db.commit()
...
async def get_user(user_id):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        db.row_factory = aiosqlite.Row
        async with db.execute(
            f"SELECT {SELECT_FIELDS} FROM users WHERE user_id = ?", (user_id,)
        ) as cursor:
...
def main_keyboard():
    kb = [
        ...
        [
            KeyboardButton(text="🥛 Сбор Молока"),
            KeyboardButton(text="💦 Полить грядку"),
        ],
        # Категории
    ...
    return ReplyKeyboardMarkup(
        keyboard=kb, resize_keyboard=True, input_field_placeholder="Главное меню"
    )
...
def town_keyboard():
    ...
    return ReplyKeyboardMarkup(
        keyboard=kb, resize_keyboard=True, input_field_placeholder="Район: Город"
    )
...
def fun_keyboard():
    ...
    return ReplyKeyboardMarkup(
        keyboard=kb, resize_keyboard=True, input_field_placeholder="Район: Развлечения"
    )
...
def upgrades_keyboard(u, info_mode=False):
    ...
    kb = [
        [
            InlineKeyboardButton(
                text=f"{icon} Бицепс ({format_num(p_click)}🍅)",
                callback_data=f"buy_click_{suffix}",
            ),
            InlineKeyboardButton(
                text=f"{icon} Сорт ({format_num(p_tomato)}🍅)",
                callback_data=f"buy_tomato_{suffix}",
            ),
        ],
        [
            InlineKeyboardButton(
                text=f"{icon} Удача ({format_num(p_luck)}🍅)",
                callback_data=f"buy_luck_{suffix}",
            ),
            InlineKeyboardButton(
                text=f"{icon} Крышка ({format_num(p_safe)}🍅)",
                callback_data=f"buy_safe_{suffix}",
            ),
        ],
        [
            InlineKeyboardButton(
                text=f"{icon} Насос ({format_num(p_eco)}🍅)",
                callback_data=f"buy_eco_{suffix}",
            ),
            InlineKeyboardButton(
                text=f"{icon} Шулер ({format_num(p_cas)}🍅)",
                callback_data=f"buy_cas_{suffix}",
            ),
        ],
        [
            InlineKeyboardButton(
                text=f"{icon} Трактор ({format_num(p_tractor)}🍅)",
                callback_data=f"buy_tractor_{suffix}",
            )
        ],
        [
            InlineKeyboardButton(
                text=f"{icon} ГМО ({format_num(p_gmo)}🍅)",
                callback_data=f"buy_gmo_{suffix}",
            )
        ],
        ...
        [
            InlineKeyboardButton(
                text=f"🔄 Обновить цены{d_text}", callback_data="refresh_upgrades"
            )
        ],
    ]
...
async def check_subscription_handler(cb: CallbackQuery):
    ...
    await cb.message.answer(
        "🎉 Добро пожаловать на Ферму! Жми кнопки меню.", reply_markup=main_keyboard()
    )
...
async def cmd_start(message: types.Message):
    ...
    try:
        ...
        await message.answer_photo(
            photo, caption=caption, reply_markup=main_keyboard(), parse_mode="HTML"
        )
    except:
        await message.answer_photo(
            DEFAULT_LOGO_URL,
    ...
    if u["tractor_level"] > 0:
        ...
        if work_time > 60:  # Минимум минута
            ...
            await message.answer(
                f"🚜 <b>ТРАКТОР ОТЧЕТ:</b>\nПока вас не было, собрано: <b>{format_num(income)}</b> 🍅",
                parse_mode="HTML",
            )
        else:
...
async def milk_handler(message: types.Message):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        async with db.execute(
            "SELECT active_boost, boost_end FROM users WHERE user_id = ?", (user_id,)
        ) as c:
...
async def plant_handler(message: types.Message):
    ...
    if random.random() < 0.20:  # Шанс снижен до 20%
        ...
        if current_mandarins > 1000000000:
            current_mandarins = 0
...
async def get_leaderboard_data(top_type="tomatoes"):
    ...
    if not res:
        ...
        kb = InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton(
                        text="🔄 Обновить", callback_data=f"top_{top_type}"
                    )
                ]
            ]
        )
    ...
    for i, row in enumerate(res):
        ...
        profile_buttons.append(
            InlineKeyboardButton(
                text=f"{i + 1} 👤", callback_data=f"view_profile_{uid}"
            )
        )
...
async def profile_new(m: types.Message):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        async with db.execute(
            "SELECT prefix, custom_status FROM users WHERE user_id = ?",
            (m.from_user.id,),
        ) as c:
...
async def code_proc(m: Message, state: FSMContext):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        # 1. Ищем код в базе
        async with db.execute(
            "SELECT uses_left, reward_type, reward_amount FROM promo_codes WHERE code = ?",
            (code_input,),
        ) as c:
        ...
        if not promo:
            await m.answer(
                "❌ Неверный код или срок действия истек.", reply_markup=main_keyboard()
            )
            await state.clear()
            return
        ...
        if uses_left == 0:
            await m.answer(
                "❌ Этот код уже активирован максимальное число раз!",
                reply_markup=main_keyboard(),
            )
            await state.clear()
            return
        ...
        async with db.execute(
            "SELECT 1 FROM used_codes WHERE user_id = ? AND code = ?",
            (user_id, code_input),
        ) as c:
        ...
        if is_used:
            await m.answer(
                "🤨 Ты уже активировал этот код!", reply_markup=main_keyboard()
            )
            await state.clear()
            return
        ...
        # Начисляем ресурс
        await db.execute(
            f"UPDATE users SET {res_type} = {res_type} + ? WHERE user_id = ?",
            (amount, user_id),
        )
...
async def daily_reward_menu(message: types.Message):
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="🔑 Открыть сундук", callback_data="open_daily_chest"
                )
            ]
        ]
    )
    ...
    try:
        ...
        await message.answer_photo(
            URL_CHEST_CLOSE, caption=caption, reply_markup=kb, parse_mode="HTML"
        )
...
async def open_chest_handler(cb: CallbackQuery):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        async with db.execute(
            "SELECT last_daily_claim FROM users WHERE user_id = ?", (user_id,)
        ) as c:
            res = await c.fetchone()
            last_claim = res[0] if res else 0
    ...
    try:
        ...
        await cb.message.answer_photo(
            URL_CHEST_OPEN, caption=final_caption, parse_mode="HTML"
        )
...
async def start_broadcast(message: types.Message, state: FSMContext):
    ...
    if (
        ...
        or message.from_user.username.lower() not in ADMINS
    ):
        return
...
async def scarecrow_event_menu(message: types.Message):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        async with db.execute(
            "SELECT last_scarecrow FROM users WHERE user_id = ?", (user_id,)
        ) as c:
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="🐦 Прогнать ворон", callback_data="scarecrow_kick"
                )
            ]
        ]
    )
    ...
    try:
        ...
        await message.answer_photo(
            media.media, caption=media.caption, reply_markup=kb, parse_mode="HTML"
        )
    except Exception as e:
...
async def scarecrow_handler(cb: CallbackQuery):
    ...
    media_good = get_scarecrow_media_safe(
        True, final_caption
    )  # True для довольного пугала
    ...
    try:
        ...
        await cb.message.answer_photo(
            media_good.media, caption=media_good.caption, parse_mode="HTML"
        )


@dp.callback_query(
    F.data == "broadcast_cancel",
    StateFilter(BroadcastStates.waiting_for_broadcast_text),
)
@dp.callback_query(
    F.data == "broadcast_cancel",
    StateFilter(BroadcastStates.waiting_for_broadcast_confirm),
)
async def broadcast_cancel(cb: CallbackQuery, state: FSMContext):
    await state.clear()
    await cb.message.edit_text("🚫 Рассылка отменена.")
    await cb.answer() @ dp.callback_query(
        F.data == "broadcast_cancel",
        StateFilter(BroadcastStates.waiting_for_broadcast_text),
    )
...
async def santa_shop(message: types.Message):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        async with db.execute(
            "SELECT mandarins FROM users WHERE user_id = ?", (message.from_user.id,)
        ) as c:
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="🎁 Большой Подарок (50 🍊)", callback_data="buy_gift_box"
                )
            ],
            [
                InlineKeyboardButton(
                    text="🧪 Ящик Химии (30 🍊)", callback_data="buy_fert_box"
                )
            ],
            [
                InlineKeyboardButton(
                    text="💰 Обменять на помидоры (10 🍊 = 500🍅)",
                    callback_data="ex_mand_tom",
                )
            ],
        ]
    )
...
async def buy_gift(cb: CallbackQuery):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        async with db.execute(
            "SELECT mandarins, tomatoes, milk, fertilizer FROM users WHERE user_id = ?",
            (user_id,),
        ) as c:
            u = await c.fetchone()
...
async def nav_town(message: types.Message):
    await message.answer(
        "🏙 Вы пришли на городскую площадь.", reply_markup=town_keyboard()
    )
...
async def show_cards_list(message: types.Message):
            ...
            await message.edit_text(
                ...
                reply_markup=InlineKeyboardMarkup(
                    inline_keyboard=[
                        [
                            InlineKeyboardButton(
                                text="⤾ Назад", callback_data="refresh_inv"
                            )
                        ]
                    ]
                ),
    ...
    for card_id, count in my_cards:
        ...
        kb_builder.append(
            [InlineKeyboardButton(text=btn_text, callback_data=f"view_card_{card_id}")]
        )
...
async def nav_fun(message: types.Message):
    await message.answer(
        "🎪 Добро пожаловать в парк развлечений!", reply_markup=fun_keyboard()
    )
...
async def admin_panel_start(message: types.Message, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="💰 Ресурсы (Give/Set)", callback_data="admin_cat_eco"
                )
            ],
            [
                InlineKeyboardButton(
                    text="🃏 Карточки (Give/Take)", callback_data="admin_cat_cards"
                )
            ],
...
async def admin_cards_menu(cb: CallbackQuery):
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="➕ ВЫДАТЬ Карту", callback_data="adm_card_op_give"
                )
            ],
            [
                InlineKeyboardButton(
                    text="➖ ЗАБРАТЬ Карту", callback_data="adm_card_op_take"
                )
            ],
...
async def admin_card_op_select(cb: CallbackQuery, state: FSMContext):
    ...
    await cb.message.edit_text(
        "✍️ <b>Введите ID КАРТЫ</b> (текстом):\nПример: <code>morgen</code>, <code>52</code>...",
        parse_mode="HTML",
    )
...
async def admin_card_get_id(message: types.Message, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="👥 ВЫДАТЬ ВСЕМ", callback_data="adm_target_all_cards"
                )
            ]
        ]
    )
    ...
    await message.answer(
        f"✅ Карта: <b>{CARDS[card_id]['name']}</b>\nТеперь введите <b>ID ИГРОКА</b> (число) или нажмите кнопку:",
        reply_markup=kb,
        parse_mode="HTML",
    )
...
# Выдача ВСЕМ (Карты)
@dp.callback_query(
    F.data == "adm_target_all_cards", StateFilter(AdminCardStates.waiting_for_target)
)
async def admin_card_target_all(cb: CallbackQuery, state: FSMContext):
    ...
    if op == "take":
        await cb.answer("❌ Нельзя забрать у всех сразу.", show_alert=True)
        return
    ...
    await cb.message.edit_text(
        f"✅ Карта <b>{card_id}</b> выдана {count} игрокам!", parse_mode="HTML"
    )
    await state.clear()
...
async def admin_card_target_single(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        f"✅ Карта <b>{card_id}</b> {action} у ID {target_id}.", parse_mode="HTML"
    )
    await state.clear()
...
async def admin_eco_menu(cb: CallbackQuery):
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="🍅 Помидоры", callback_data="adm_res_tomatoes"
                ),
                InlineKeyboardButton(text="🥛 Молоко", callback_data="adm_res_milk"),
            ],
            [
                InlineKeyboardButton(
                    text="🍊 Мандарины", callback_data="adm_res_mandarins"
                ),
                InlineKeyboardButton(
                    text="🧪 Химия", callback_data="adm_res_fertilizer"
                ),
            ],
            [InlineKeyboardButton(text="🔙 Назад", callback_data="admin_back_main")],
        ]
...
async def admin_select_resource(cb: CallbackQuery, state: FSMContext):
    ...
    await cb.message.edit_text(
        f"⚙️ Ресурс: <b>{res.upper()}</b>. Действие:", reply_markup=kb, parse_mode="HTML"
    )
...
async def admin_select_op(cb: CallbackQuery, state: FSMContext):
    ...
    if op in ["add", "remove"]:
        kb = InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton(
                        text="👥 ПРИМЕНИТЬ КО ВСЕМ", callback_data="adm_target_all_res"
                    )
                ]
            ]
        )
    ...
    await cb.message.edit_text(
        "✍️ <b>Введите ID ИГРОКА</b> или нажмите кнопку:",
        reply_markup=kb,
        parse_mode="HTML",
    )
...
# Кнопка ВСЕМ (Ресурсы)
@dp.callback_query(
    F.data == "adm_target_all_res", StateFilter(AdminEcoStates.waiting_for_user_id)
)
async def admin_res_all(cb: CallbackQuery, state: FSMContext):
    ...
    await cb.message.edit_text(
        "✍️ <b>Введите КОЛИЧЕСТВО</b> для всех:", parse_mode="HTML"
    )
...
async def admin_res_final(message: types.Message, state: FSMContext):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        if target == "ALL":
            if op == "add":
                await db.execute(f"UPDATE users SET {res} = {res} + ?", (amount,))
            elif op == "remove":
                await db.execute(
                    f"UPDATE users SET {res} = MAX(0, {res} - ?)", (amount,)
                )
            await db.commit()
            await message.answer(
                f"✅ <b>{op.upper()} {amount} {res}</b> выполнено для ВСЕХ.",
                parse_mode="HTML",
            )
        else:
            # Одиночное действие
            if op == "add":
                await db.execute(
                    f"UPDATE users SET {res} = {res} + ? WHERE user_id = ?",
                    (amount, target),
                )
            elif op == "remove":
                await db.execute(
                    f"UPDATE users SET {res} = MAX(0, {res} - ?) WHERE user_id = ?",
                    (amount, target),
                )
            elif op == "set":
                await db.execute(
                    f"UPDATE users SET {res} = ? WHERE user_id = ?", (amount, target)
                )
            await db.commit()
...
async def view_card_handler(cb: CallbackQuery):
    try:
        ...
        card_id = cb.data.split("_", 2)[
            2
        ]  # split(_, 2) чтобы не ломалось если в ID есть "_"
        user_id = cb.from_user.id
...
async def view_other_profile(cb: CallbackQuery):
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="📂 Коллекция игрока",
                    callback_data=f"view_collection_{target_id}",
                )
            ],
...
async def cmd_hide(message: types.Message):
    ...
    if len(args) < 2:
        await message.answer(
            "⚠️ Используйте: `/hide admins` или `/hide admin <user>`",
            parse_mode="Markdown",
        )
        return
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        if target_type == "admins":
            ...
            await db.commit()
            await message.answer(
                "🕵️‍♂️ <b>ОПЕРАЦИЯ ВЫПОЛНЕНА:</b>\nВсе администраторы скрыты из топов.",
                parse_mode="HTML",
            )
        ...
        # 2. Скрыть КОНКРЕТНОГО игрока
            ...
            async with db.execute(
                f"SELECT username FROM users WHERE {where_clause}", (val,)
            ) as c:
                user = await c.fetchone()

            if user:
                await db.execute(
                    f"UPDATE users SET is_hidden = 1 WHERE {where_clause}", (val,)
                )
                await db.commit()
                await message.answer(
                    f"✅ Игрок <b>{user[0]}</b> скрыт из топов.", parse_mode="HTML"
                )
            else:
...
async def cmd_unhide(message: types.Message):
    ...
    if len(args) < 2:
        await message.answer(
            "⚠️ Используйте: `/unhide admins` или `/unhide admin <user>`",
            parse_mode="Markdown",
        )
        return
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        if target_type == "admins":
            ...
            await db.commit()
            await message.answer(
                "👁 <b>ОПЕРАЦИЯ ВЫПОЛНЕНА:</b>\nАдминистраторы снова видны в топах.",
                parse_mode="HTML",
            )
        ...
        # 2. Раскрыть КОНКРЕТНОГО игрока
            ...
            if user:
                await db.execute(
                    f"UPDATE users SET is_hidden = 0 WHERE {where_clause}", (val,)
                )
                await db.commit()
                await message.answer(
                    f"✅ Игрок <b>{user[0]}</b> возвращен в топы.", parse_mode="HTML"
                )
            else:
                await message.answer("❌ Игрок не найден.")
...
async def get_card_keyboard(current_id, user_id, is_owner, target_id_if_not_owner=None):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        async with db.execute(
            "SELECT card_id FROM user_cards WHERE user_id = ?", (user_id,)
        ) as c:
    ...
    if current_id in all_cards:
        ...
        prev_card = (
            all_cards[idx - 1] if idx > 0 else all_cards[-1]
        )  # Круговая навигация
        ...
        if is_owner:
            btn_prev = InlineKeyboardButton(
                text="⬅️", callback_data=f"view_card_{prev_card}"
            )
            btn_next = InlineKeyboardButton(
                text="➡️", callback_data=f"view_card_{next_card}"
            )
        else:
            # target_id_if_not_owner - это ID того, чьи карты мы смотрим
            btn_prev = InlineKeyboardButton(
                text="⬅️",
                callback_data=f"peek_card_{target_id_if_not_owner}_{prev_card}",
            )
            btn_next = InlineKeyboardButton(
                text="➡️",
                callback_data=f"peek_card_{target_id_if_not_owner}_{next_card}",
            )
        ...
        kb_rows.append(
            [
                btn_prev,
                InlineKeyboardButton(
                    text=f"{idx + 1}/{len(all_cards)}", callback_data="ignore"
                ),
                btn_next,
            ]
        )
    ...
    if is_owner:
        ...
        kb_rows.append(
            [
                InlineKeyboardButton(
                    text="💰 Продать", callback_data=f"sell_init_{current_id}"
                )
            ]
        )
        kb_rows.append(
        ...
        kb_rows.append(
            [
                InlineKeyboardButton(
                    text="⤾ К профилю игрока",
                    callback_data=f"view_profile_{target_id_if_not_owner}",
                )
            ]
        )
...
async def render_card_message(message_or_call, card_id, count, is_owner, owner_id):
    ...
    kb = await get_card_keyboard(
        card_id, owner_id, is_owner, owner_id if not is_owner else None
    )
        ...
        try:
            if media:
                await message_or_call.message.edit_media(
                    media=InputMediaPhoto(
                        media=media, caption=caption, parse_mode="HTML"
                    ),
                    reply_markup=kb,
                )
            else:
            ...
            if media:
                await message_or_call.message.answer_photo(
                    media, caption=caption, reply_markup=kb, parse_mode="HTML"
                )
            else:
...
async def view_other_collection(cb: CallbackQuery):
    ...
    for card_id, count in target_cards:
        if card_id not in CARDS:
            continue
        card_data = CARDS[card_id]
        rarity_icon = RARITY_INFO.get(
            card_data.get("rarity", "common"), RARITY_INFO["common"]
        )["icon"]
        ...
        kb_builder.append(
            [
                InlineKeyboardButton(
                    text=btn_text, callback_data=f"peek_card_{target_id}_{card_id}"
                )
            ]
        )
    ...
    kb_builder.append(
        [
            InlineKeyboardButton(
                text="⤾ К профилю", callback_data=f"view_profile_{target_id}"
            )
        ]
    )
...
async def shop_menu(message: types.Message):
    ...
    await message.answer(
        text, reply_markup=upgrades_keyboard(user, info_mode=False), parse_mode="HTML"
    )
...
async def switch_shop_mode(cb: CallbackQuery):
    ...
    try:
        await cb.message.edit_reply_markup(
            reply_markup=upgrades_keyboard(user, info_mode=is_info)
        )
    except:
        pass
...
async def buy_upgrade(cb: CallbackQuery):
    ...
    mode = parts[2]  # b (buy) или i (info)

    # === 1. РЕЖИМ ИНФОРМАЦИИ (i) ===
    if mode == "i":
        descriptions = {
            "click": "💪 <b>Бицепс</b>\nТренирует руки.\nЭффект: +1 Молока за клик.",
            "tomato": "🧬 <b>Сорт</b>\nГенная инженерия.\nЭффект: Шанс двойного урожая помидоров.",
            "luck": "🍀 <b>Удача</b>\nКлевер.\nЭффект: Повышает шанс найти химию.",
            "safe": "🛡 <b>Крышка</b>\nГерметичность.\nЭффект: Снижает шанс пролить молоко.",
            "eco": "📉 <b>Насос</b>\nАвтоматика.\nЭффект: Снижает стоимость полива.",
            "cas": "🃏 <b>Шулер</b>\nКарты в рукаве.\nЭффект: Снижает цену ставки в казино.",
            "gmo": "🧪 <b>ГМО</b>\nЭксперименты.\nЭффект: Шанс вернуть 50% молока при поливе.",
            "tractor": "🚜 <b>Трактор</b>\nАвто-фарм.\nЭффект: Собирает помидоры, пока ты оффлайн.",
        }
    ...
    if tom >= cost:
        ...
        await cb.answer("✅ Куплено!", show_alert=False)
        ...
        await cb.answer(
            f"❌ Не хватает помидоров! Нужно {format_num(cost)}", show_alert=True
        )
...
async def lab_menu(message: types.Message):
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text=f"🧪 Купить Мутаген ({format_num(MUTAGEN_SHOP_PRICE)} 🍅)",
                    callback_data="buy_mutagen",
                )
            ],
            [
                InlineKeyboardButton(
                    text="⚗️ Начать Синтез", callback_data="start_craft_list"
                )
            ],
...
async def craft_list_handler(cb: CallbackQuery):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        async with db.execute(
            "SELECT card_id, count FROM user_cards WHERE user_id = ? AND count >= ?",
            (user_id, CRAFT_CARDS_NEEDED),
        ) as c:
            candidates = await c.fetchall()

    if not candidates:
        await cb.answer(
            f"❌ Нет подходящих карт (нужно {CRAFT_CARDS_NEEDED} копии)",
            show_alert=True,
        )
        return
    ...
    for card_id, count in candidates:
        ...
        kb_rows.append(
            [InlineKeyboardButton(text=btn_text, callback_data=f"do_craft_{card_id}")]
        )
...
async def execute_craft(cb: CallbackQuery):
    ...
    target_rarity = "rare"
    if input_rarity == "rare":
        target_rarity = "epic"
    elif input_rarity == "epic":
        target_rarity = "limited"

    potential_rewards = [
        cid for cid, cdata in CARDS.items() if cdata.get("rarity") == target_rarity
    ]
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        await db.execute(
            "UPDATE user_cards SET count = count - ? WHERE user_id = ? AND card_id = ?",
            (CRAFT_CARDS_NEEDED, user_id, card_id_input),
        )
        await db.execute(
            "UPDATE users SET mutagen = mutagen - ? WHERE user_id = ?",
            (CRAFT_COST_MUTAGEN, user_id),
        )

        exists = await db.execute_fetchall(
            "SELECT 1 FROM user_cards WHERE user_id = ? AND card_id = ?",
            (user_id, reward_card_id),
        )
        if exists:
            await db.execute(
                "UPDATE user_cards SET count = count + 1 WHERE user_id = ? AND card_id = ?",
...
async def admin_console_loop(bot: Bot):
    ...
    print(
        f"║ {Fore.LIGHTGREEN_EX}👑 MOLOKO FARM запущен нах v4.0                                   {Fore.LIGHTBLUE_EX}║"
    )
    print(
        f"║ {Fore.WHITE}Управление: sql, logs, maint, ban, give, list, stats         {Fore.LIGHTBLUE_EX}║"
    )
    print(
        f"╚══════════════════════════════════════════════════════════════╝{Style.RESET_ALL}"
    )
        ...
        try:
            ...
            # === CHECK (Полная инфа об игроке) ===
                ...
                async with aiosqlite.connect(DB_NAME) as db:
                    ...
                    async with db.execute(
                        f"SELECT * FROM users WHERE {query_col} = ?", (query_val,)
                    ) as c:
                        user = await c.fetchone()
                ...
                if not user:
                    ...
                    for key in user.keys():
                        ...
                        if key == "milk":
                            color = Fore.WHITE
                        if key == "is_admin" and val == 1:
                            val = f"{Fore.GREEN}YES{Style.RESET_ALL}"
                        if key == "is_banned" and val == 1:
                            val = f"{Fore.RED}YES{Style.RESET_ALL}"
            ...
            # === CODE (Управление промокодами) ===
                ...
                if subcmd == "list":
                    ...
                    if not codes:
                        print("Нет активных кодов.")

                    for row in codes:
                        c_name, uses, r_type, r_amount = row
                        uses_str = "♾ Бесконечно" if uses == -1 else f"{uses} шт."
                        print(
                            f"🔹 {Fore.YELLOW}{c_name}{Style.RESET_ALL} -> Дает: {r_amount} {r_type} | Осталось: {uses_str}"
                        )
                ...
                # code create OMEGA -1 milk 1000
                    ...
                    if len(parts) < 6:
                        print(
                            "❌ Формат: code create <код> <кол-во/-1> <ресурс> <сумма>"
                        )
                        continue
                    ...
                    async with aiosqlite.connect(DB_NAME) as db:
                        try:
                            await db.execute(
                                ...
                                (c_name, c_uses, c_res, c_amount),
                            )
                            await db.commit()
                            print(
                                f"{Fore.GREEN}✅ Код {c_name} создан!{Style.RESET_ALL}"
                            )
                        except Exception as e:
                            print(
                                f"{Fore.RED}❌ Ошибка (такой код уже есть?): {e}{Style.RESET_ALL}"
                            )
                ...
                # --- CODE DELETE ---
                    ...
                    async with aiosqlite.connect(DB_NAME) as db:
                        await db.execute(
                            "DELETE FROM promo_codes WHERE code = ?", (c_name,)
                        )
                        await db.execute(
                            "DELETE FROM used_codes WHERE code = ?", (c_name,)
                        )  # Удаляем историю ввода, если надо
                        await db.commit()
            ...
            # === 2. РЕЖИМ ТЕХРАБОТ ===
                ...
                status = (
                    f"{Fore.RED}АКТИВЕН (Игроки заблокированы){Style.RESET_ALL}"
                    if MAINTENANCE_MODE
                    else f"{Fore.GREEN}ВЫКЛЮЧЕН (Игра идет){Style.RESET_ALL}"
                )
            ...
            # === 3. SETADMIN (Выдать админку) ===
                ...
                if len(parts) < 3:
                    print(
                        f"{Fore.YELLOW}📝 Юз: setadmin <username> <1/0>{Style.RESET_ALL}"
                    )
                    continue
                ...
                try:
                    ...
                    print("❌ Значение должно быть 1 или 0")
                    continue
                ...
                async with aiosqlite.connect(DB_NAME) as db:
                    ...
                    if not res:
                        print(
                            f"{Fore.RED}❌ Игрок @{target_name} не найден в БД.{Style.RESET_ALL}"
                        )
                        continue

                    uid = res[0]
                    await db.execute(
                        "UPDATE users SET is_admin = ? WHERE user_id = ?", (lvl, uid)
                    )
                    await db.commit()
                ...
                if lvl == 1:
                    ...
                    print(
                        f"{Fore.GREEN}✅ Пользователь {target_name} теперь АДМИН!{Style.RESET_ALL}"
                    )
                    try:
                        await bot.send_message(
                            uid, "😎 <b>ВАМ ВЫДАНЫ ПРАВА АДМИНИСТРАТОРА!</b>"
                        )
                    except:
                        pass
                else:
                    ...
                    print(
                        f"{Fore.YELLOW}🔸 С пользователя {target_name} сняты права.{Style.RESET_ALL}"
                    )
            ...
            # === 4. BC (Broadcast - Рассылка) ===
                ...
                print(
                    f"{Fore.GREEN}✅ Рассылка завершена. Доставлено: {count} чел.{Style.RESET_ALL}"
                )
            ...
            # === 5. SAY / MSG (Личное сообщение) ===
                ...
                if target_id:
                    try:
                        ...
                        print(
                            f"{Fore.GREEN}✅ Отправлено игроку {target_id}{Style.RESET_ALL}"
                        )
                    except Exception as e:
                        print(
                            f"{Fore.RED}❌ Не удалось отправить (блок?): {e}{Style.RESET_ALL}"
                        )
                else:
                    print(f"{Fore.RED}❌ Игрок не найден.{Style.RESET_ALL}")
            ...
            # === 6. SET (Установить значение) ===
                ...
                if len(parts) < 4:
                    print(
                        f"{Fore.YELLOW}📝 Юз: set <id/nick> <field> <value>{Style.RESET_ALL}"
                    )
                    print(
                        "Доступные поля: tomatoes, milk, fertilizer, mandarins, click_level, luck_level..."
                    )
                    continue
                ...
                if field not in allowed_fields:
                    print(
                        f"{Fore.RED}❌ Поле '{field}' менять нельзя! Только ресурсы и уровни.{Style.RESET_ALL}"
                    )
                    continue
                ...
                print(
                    f"{Fore.GREEN}✅ Игроку {target_id} установлено {field} = {value}{Style.RESET_ALL}"
                )
                    ...
                    async with aiosqlite.connect(DB_NAME) as db:
                        async with db.execute(query) as cursor:
                            ...
                            if query.strip().upper().startswith("SELECT"):
                                ...
                                for row in rows:
                                    print(
                                        " | ".join(f"{str(item):<12}" for item in row)
                                    )
                                print(
                                ...
                                await db.commit()
                                print(
                                    f"{Fore.GREEN}✅ Запрос выполнен успішно! Изменений в БД.{Style.RESET_ALL}"
                                )
                except Exception as e:
            ...
            # === 4. LIST (Обновленный) ===
                ...
                async with aiosqlite.connect(DB_NAME) as db:
                    async with db.execute(
                        "SELECT user_id, username, tomatoes, milk, last_active FROM users ORDER BY last_active DESC LIMIT 20"
                    ) as c:
                        users = await c.fetchall()
                ...
                print(
                    f"{'ID':<12} | {'Имя':<15} | {'Помидоры':<10} | {'Молоко':<10} | {'Статус'}"
                )
                print("-" * 65)
                ...
                for u in users:
                    ...
                    if (now - last) < 300:
                        ...
                        status = f"{Fore.YELLOW}1h ago{Style.RESET_ALL}"
                    else:
                        status = f"{Fore.RED}OFFLINE{Style.RESET_ALL}"

                    print(
                        f"{uid:<12} | {name:<15} | {format_num(tom):<10} | {format_num(milk):<10} | {status}"
                    )
            ...
            # === 5. GIVE (Улучшенный) ===
                ...
                print(
                    f"{Fore.GREEN}✅ Выдано {amount} {res} игроку {target}{Style.RESET_ALL}"
                )
                try:
                    await bot.send_message(
                        target,
                        f"🎁 <b>АДМИН:</b> Вам начислено {format_num(amount)} {res}!",
                    )
                except:
                    pass
            ...
            # === 7. STATS ===
                ...
                async with aiosqlite.connect(DB_NAME) as db:
                    ...
                    money = (
                        await (
                            await db.execute("SELECT sum(tomatoes) FROM users")
                        ).fetchone()
                    )[0]
                ...
                print(
                    f"💰 Всего помидоров: {Fore.GREEN}{format_num(money)}{Style.RESET_ALL}"
                )
...
def get_academy_render_data(u, harvest_msg=""):
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="⬆️ Повысить квалификацию", callback_data="acad_upgrades"
                )
            ],
            [
                InlineKeyboardButton(
                    text="🔄 Обновить данные", callback_data="acad_refresh"
                )
            ],
        ]
    )
...
async def acad_upgrades_menu(cb: CallbackQuery, user_data=None):
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text=f"📈 Улучшить ({format_num(price_man)} 🍅)",
                    callback_data=f"acad_buy_man_{price_man}",
                )
            ],
            [
                InlineKeyboardButton(
                    text=f"⏳ Улучшить ({format_num(price_log)} 🍅)",
                    callback_data=f"acad_buy_log_{price_log}",
                )
            ],
            [
                InlineKeyboardButton(
                    text=f"🧬 Улучшить ({format_num(price_agr)} 🍅)",
...
async def buy_course_handler(cb: CallbackQuery):
    ...
    if u["tomatoes"] < price:
        await cb.answer(
            f"❌ Не хватает помидоров! Нужно {format_num(price)}", show_alert=True
        )
        return
...
def get_academy_stats(u: aiosqlite.Row):
    ...
    if total_lvl == 0:
        ...
        title = "Студент"  # 1-4 уровня
    elif total_lvl < 15:
        title = "Бакалавр"  # 5-14 уровней
    elif total_lvl < 30:
        title = "Магистр"  # 15-29 уровней
    else:
        title = "Профессор"  # 30+ уровней
...
async def santa_shop_menu(cb: CallbackQuery):
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="🍅 1 000 Помидоров (50 кг 🍊)",
                    callback_data="santa_buy_tomatoes",
                )
            ],
            [
                InlineKeyboardButton(
                    text="🥛 500 Молока (50 кг 🍊)", callback_data="santa_buy_milk"
                )
            ],
            [
                InlineKeyboardButton(
                    text="🧪 5 шт. Химии (100 кг 🍊)", callback_data="santa_buy_fert"
                )
            ],
            [
                InlineKeyboardButton(
                    text="🃏 Карточка (200 кг 🍊)", callback_data="santa_buy_card"
                )
            ],
            [InlineKeyboardButton(text="⤾ Назад в Склад", callback_data="refresh_inv")],
        ]
...
async def nav_back_to_town(message: types.Message):
    await message.answer(
        "🏡 Вы вернулись на городскую площадь.", reply_markup=town_keyboard()
    )
...
async def nav_back(message: types.Message):
    ...
    if (
        ...
        and (
            "Город" in message.reply_to_message.text
            or "Развлечения" in message.reply_to_message.text
        )
    ):
...
async def main():
    ...
    print(
        f"{Fore.GREEN}✅ Бот 🎄 Новогоднее Обновление: Операция Оливье запущен!{Style.RESET_ALL}"
    )
